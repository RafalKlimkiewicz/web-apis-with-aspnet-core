...
namespace MyBGList.GraphQL
{
    public class Mutation
    {
        ...
        {
            var boardgame = await context.BoardGames.Where(b => b.Id == model.Id).FirstOrDefaultAsync();
        ...
        {
            var boardgame = await context.BoardGames.Where(b => b.Id == id).FirstOrDefaultAsync();
        ...
        {
            var domain = await context.Domains.Where(d => d.Id == model.Id).FirstOrDefaultAsync();
        ...
        {
            var domain = await context.Domains.Where(d => d.Id == id).FirstOrDefaultAsync();
        ...
        {
            var mechanic = await context.Mechanics.Where(m => m.Id == model.Id).FirstOrDefaultAsync();
        ...
        {
            var mechanic = await context.Mechanics.Where(m => m.Id == id).FirstOrDefaultAsync();
